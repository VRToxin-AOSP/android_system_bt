...
static BOOLEAN bta_dm_check_av(UINT16 event)
{
    ...
    /* Avoid role switch upon inquiry if A2DP is actively streaming, as it
       introduces an audio glitch due to FW scheduling delays (unavoidable). */
    if (event == BTA_DM_API_SEARCH_EVT)
        return FALSE;

    APPL_TRACE_WARNING("%s cur_av_count: %d", __func__, bta_dm_cb.cur_av_count);
    if (bta_dm_cb.cur_av_count == 0)
        return FALSE;

    for(int i = 0; i < bta_dm_cb.device_list.count; i++)
    {
        const tBTA_DM_PEER_DEVICE *p_dev = &bta_dm_cb.device_list.peer_device[i];
        if ((p_dev->conn_state == BTA_DM_CONNECTED) && (p_dev->info & BTA_DM_DI_AV_ACTIVE))
        {
            /* make master and take away the role switch policy */
            int rc = BTM_SwitchRole(p_dev->peer_bdaddr, HCI_ROLE_MASTER, (tBTM_CMPL_CB *)bta_dm_rs_cback);
